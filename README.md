...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/compliance_index \
  -H 'Content-Type: application/json' \
  -d '{"query":"GDPR"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/compliance_index`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
